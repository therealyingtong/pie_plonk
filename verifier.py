...
from kzg.setup import G1Point, G2Point
from compiler.program import Program
from compiler.utils import Column
from kzg.setup import Setup
from curve import Scalar, ec_lincomb, ec_mul
...
class VerificationKey:
    ...
    def verify(self, group_order: int, proof, public=[], optimized=True) -> bool:
        ...
        _fft_cofactor = transcript.squeeze()
    ...
    def make_verification_key(cls, program: Program, setup: Setup):
        ...
        return cls(
            ...
            setup.X2,
            Scalar.root_of_unity(program.group_order)
        )
    ...
    def _verify_inner(
        ...
        assert b.pairing(
            ...
            b.add(self.X_2, ec_mul(b.G2, -zed * self.root_of_unity)),
            proof['W_zw_1']
        )
        print("done check 2")
        return True
    ...
    def _optimized_verify_inner(
        ...
        assert b.pairing(self.X_2, ec_lincomb([
            ...
            (proof['W_z_1'], zed),
            (proof['W_zw_1'], u * zed * self.root_of_unity),
            (F_pt, 1),
            (E_pt, -1)
        ]))
